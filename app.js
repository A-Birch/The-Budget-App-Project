...
var budgetController = (function() {
	...
	return {
		...
		deleteItem: function(type, id) {
			...
			// id = 6
			// data.allItems[type][id]; // doesn't work in this case
			// ids = [1 2 4 6 8]
			// index = 3
...
var UIController = (function(){
	...
	var formatNumber = function(num, type) {
			...
			*/
			...
			/*
			if (int.length > 3) {
				int = int.substring(0, int.length - 3) + ',' + int.substring(int.length - 3, 3); // input 2450, output 2,450
			}
			*/
			dec = numSplit[1];
	...
	return {
		init: function() {
			...
			UICtrl.displayBudget({
				...
				totalExp: 0,
				percentage: -1
			}); // for reset all values in the UI when starting app in the first time
			setupEventListeners();
		}
	};
